...
(() => {
  const slider = document.querySelector(".olif-slider");

  if (!slider || !window.gsap) {
    return;
  }

  const slides = Array.from(slider.querySelectorAll(".olif-slide"));
  const dotsContainer = slider.querySelector(".olif-slider__dots");
  const previousButton = slider.querySelector(".olif-slider__previous");
  const nextButton = slider.querySelector(".olif-slider__next");
  const progress = slider.querySelector(".olif-slider__progress span");

  if (slides.length < 2) {
    return;
  }

  const autoplayDuration = 6;
  const reduceMotion = window.matchMedia(
    "(prefers-reduced-motion: reduce)"
  ).matches;
  ...
  slides.forEach((slide, index) => {
    ...
    dot.type = "button";
    dot.className = "olif-slider__dot";
  ...
  const dots = Array.from(
    dotsContainer.querySelectorAll(".olif-slider__dot")
  );
  ...
  const showSlide = (nextIndex, direction) => {
    ...
    const currentContent = currentSlide.querySelector(
      ".olif-slide__content"
    );
    const nextContent = nextSlide.querySelector(
      ".olif-slide__content"
    );
    ...
    const timeline = gsap.timeline({
      onComplete: () => {
        ...
        gsap.set(
          [currentContent, currentImage, nextSlide, nextContent, nextImage],
          { clearProps: "all" }
        );
  ...
  gsap.fromTo(
    slides[0].querySelectorAll(".olif-slide__content > *"),
    {
      opacity: 0,
      y: 35
    },
